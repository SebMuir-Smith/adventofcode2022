...
public class Day14Part2 {
    ...
    public static void main(String[] args) {
        Coord start = new Coord("500,0");
        start.updateMaximums();
        List<List<Coord>> coords = input.lines().map(line -> Arrays.stream(line.replaceAll(" ", "").split("->")).map(Coord::new).toList()).toList();
        coords.forEach(l -> l.stream().forEach(Coord::updateMaximums));
        Coord.calculateRanges();
        coords.forEach(list -> {
            IntStream.iterate(0, i -> i + 1).takeWhile(i -> i + 1 < list.size()).forEach(i -> list.get(i).drawTo(list.get(i + 1)));
        });

        Coord.draw(500, 0, '+');
        Coord.displayScreen();
        int sandCounter = 0;
        while (start.moveSand()){
            sandCounter++;
            //Coord.displayScreen();
            start = new Coord("500,0");
        }
    ...
    static class Coord {
        ...
        static void calculateRanges() {
            yMax = yMax + 2;
            yRange = yMax - yMin + 1;
            xMax =  Math.max(500 + yMax, xMax);
            xMin = Math.min(500 - yMax, xMin);
            xRange = xMax - xMin + 1;
        ...
        public void drawTo(Coord to) {
            int xStep = to.x != x ? (to.x - x) / (abs(to.x - x)) : 0;
            int yStep = to.y != y ? (to.y - y) / (abs(to.y - y)) : 0;

            draw(to.x,to.y, '#');

            for (int xDraw = x; xDraw != to.x; xDraw += xStep) {
                draw(xDraw, y, '#');
            }

            for (int yDraw = y; yDraw != to.y; yDraw += yStep) {
                draw(x, yDraw, '#');
            }
        }
        ...
        static boolean isClear(int x, int y){
            return inBounds(x,y+1) && screen[(y - yMin)][(x - xMin)] == '.';
        }

        boolean moveSand(){

                if (screen[(0 - yMin)][(500 - xMin)] != '+'){
                    return false;
                }
                if (isClear(x, y + 1)) {
                    y += 1;
                    return moveSand();
                } else if (isClear(x - 1, y + 1)) {
                    y += 1;
                    x -= 1;
                    return moveSand();
                } else if (isClear(x + 1, y + 1)) {
